...
class BotQLearning(Bot):
	...
	def __get_action_by_policy(self, xdif, ydif, vel):
		...
		max = float("-inf")
		action_to_take = 0
		for key, value in self.__variations.items():
			if max <= value:
	...
	def __update_qvalues(self):
		...
		good_idxs = list(range(LAST_STATES_PENALIZED + upper_pipe_hit, len(history)))
		bad_idxs = list(range(0, LAST_STATES_PENALIZED + upper_pipe_hit))
